...
cdef class VNode(TreeNode):
    ...
    def __str__(self):
        return f'VNode<N:{self.num_visits}, V:{self.value} | num_children: {len(self.children)}>'
    ...
    def children_data(self):
        ...
        for action in self.children:
            string += f' |_{action}, N:{self[action].num_visits}, V:{self[action].value}\n'
        return string
...
cdef class POUCT(Planner):
    ...
    Args:
        ...
        num_sims (int): Number of simulations for each planning step. If negative,
            ...
            If both `num_sims` and `planning_time` are negative, then the planner will test_utils for 1 second.
        rollout_policy (PolicyModel): rollout policy.
    ...
    cpdef set_rollout_policy(self, PolicyModel rollout_policy):
        """
        set_rollout_policy(self, PolicyModel rollout_policy)
    ...
    cpdef _simulate(POUCT self,
        ...
        # This section of code is modified from original POUCT
        # to accommodate for resampling during update.
        # --------------------------------------------------------------------
        if root is None:
    ...
    cpdef _rollout(self, State state, tuple history, VNode root, int depth):
        ...
        cdef float total_discounted_reward = 0.0
        ...
        while depth < self._max_depth:
            ...
            """Exception handling added to handle macro actions."""
            #---------------------------------------------------------------------------------------------------------
            # try:
            next_state, observation, reward, nsteps = self._agent.blackbox_model.sample(state,
                                                                                        action,
                                                                                        self.discount_factor)
            # except:
            #     """Original logic without exception handling..."""
            #     next_state, observation, reward, nsteps = sample_generative_model(self._agent, state, action)
            #---------------------------------------------------------------------------------------------------------
