...
cdef class QNode(TreeNode):
    ...
    def __init__(self, num_visits=0, R=0.0, D=0.0):
        super().__init__()
    ...
    def __str__(self):
        return typ.red(f'QNode<N:{self.num_visits}, R:{self.R}, D:{self.D} | {self.children.keys()}>')
...
cdef class VNode(TreeNode):
    ...
    def __init__(self, num_visits=0, V=0.0, W=1.0):
        super().__init__()
        self.num_visits = num_visits
        self.V = V
        self.W = W
    ...
    def __str__(self):
        return f'VNode<N:{self.num_visits}, V:{self.V}, W:{self.W} | num_children: {len(self.children)}>'
    ...
    def children_data(self):
        ...
        for action in self.children:
            string += (f' |_{action}, N:{self[action].num_visits}, '
                       f'R:{round(self[action].R, 2)}, D:{round(self[action].D, 3)}, '
                       f'Pol:{round(self.policy[action], 3)}\n')
        return string

cdef class VNodeParticles(VNode):
    def __init__(self, num_visits=0, V=0.0, W=1.0, belief=Particles([])):
        super().__init__(num_visits, V, W)
        self.belief = belief

    def __str__(self):
        return f'VNode<N:{self.num_visits}, V:{self.V}, W:{self.W}, Num Particles:{len(self.belief)} | num_children: {len(self.children)}>'
...
cdef class RefSolver(Planner):
    ...
    """
    The RefSolver used in Liang et al. (ISRR2024).
    ...
    Args:
        ...
        temperature (float): The inverse of the weight given to the KL-penalty (Default: 1.)
        num_visits_init (int): The default number of initial visits for a new node (Default: 0)
    ...
    def __init__(self,
                 ...
                 discount_factor=0.99, temperature=1.,
    ...
    cpdef update_policy(self, VNode root):
        ...
        normalizer = sum([root[ma].num_visits / root.num_visits
                          * np.exp(self._temperature * (root[ma].R + self._discount_factor * root[ma].D))
                          for ma in root.children])
        root.policy = {ma: root[ma].num_visits / root.num_visits *
                           np.exp(self._temperature * (root[ma].R + self._discount_factor * root[ma].D))
                           / normalizer
                       for ma in root.children}
    ...
    cpdef _simulate(self, State state, tuple history, VNode root, QNode parent, Observation observation, int depth):
        ...
        return np.log(self._maintain_expectation(root, action, observation, next_state, reward,
                                                 history + ((action, observation),), depth, nsteps)) / self._temperature
    ...
    cpdef _maintain_expectation(self, VNode root, Action action, Observation observation,
        ...
        ma_len = len(action.action_sequence) if isinstance(action, MacroAction) else 1

        w = root.num_visits * root.W - root[action].num_visits * np.exp(self._temperature *
                                                      (root[action].R + self._discount_factor ** ma_len * root[action].D))
        p = root[action].num_visits * root[action].D - root[action][observation].num_visits * root[action][observation].V
        ...
        v = self._simulate(next_state,
                  ...
                  observation,
                  depth + nsteps)
        root[action].D = (p + root[action][observation].num_visits * v) / root[action].num_visits
        root.num_visits += 1
        root.W = (w + root[action].num_visits * np.exp(self._temperature *
                                             (root[action].R + self._discount_factor ** ma_len * root[action].D))) / root.num_visits
        return root.W
