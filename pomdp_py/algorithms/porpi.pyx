...
cdef class QNode(TreeNode):
    ...
    def __init__(self, num_visits=0, R=0.0, D=0.0, pref=0.0):
        ...
        self.R = R
        self.D = D # TODO: Not required anymore.
        self.pref = pref
        self.children = {}
...
cdef class PORPI(Planner):
    ...
    Args:
        ...
        exploration_const (float): The exploration constant for UCB. Default: 500.
        ...
        for action in root.children:
            if root[action].num_visits == 0: # TODO: Probably redundant.
                ...
                val = root[action].pref + \
                    self._exploration_const * np.sqrt(np.log(root.num_visits) / root[action].num_visits)
    ...
    cpdef float _simulate(PORPI self,
        ...
        if depth > self._max_depth:
            if hasattr(self._agent._policy_model, "value_heuristic"):
                return self._agent._policy_model.value_heuristic(state, next_state=None,
                                                       reward=0, discount=self.discount_factor) * self._reward_scale
            else:
        ...
        # action = root.sample_softmax(self._exploitation_temp, "pref") if np.random.random() <= self._exploitation_prob and len(root.children) else root.least_visited()

        if root[action] is None and len(root.children) < self._action_branching_factor:
        # if root[action] is None:
            root[action] = QNode()
            # print("Initialised new node.")
        # TODO: Probably can make more efficient.
        ...
        root[action].num_visits += 1
        root[action].R += (reward * self._reward_scale - root[action].R) / root[action].num_visits
        root[action].D += (self._simulate(next_state,
                                           history + ((action, observation),),
                                           root[action][observation],
                                           root[action],
                                           observation,
        ...
        #                                    depth + nsteps)
        root.num_visits += 1
        root[action].pref = root[action].pref - root.V + root[action].R + (self._discount_factor ** nsteps) * root[action].D
        ...
        # print(f"Pref: {root[action].pref} | D: {root[action].D} | R: {root[action].R}| num_visits: {root[action].num_visits} | depth: {depth}")
