...
cdef class PORPI(Planner):
    ...
    """PORPI (Partially-Observed Reference Policy Iteration) constructs the POMDP solution by
    solving a family of Reference-Based POMDPs, whose reference-policies are the solution to the
    previous iteration.

    # TODO: Tidy parameters once the planner is bedded down.
    ...
    Args:
        ...
        action_branching_factor (int): The maximum number of actions per belief node. Default: 5.
        ...
        sir_temp (float): A positive real number indicating the temperature of the softmax used to exaggerate
            resampling weights. Default: 0.5.
        ...
        reward_scale (float): A scale factor for the reward. Default: 1.0.
    ...
    MIN_EXPONENT = -746 # The smallest exponent numpy can accept. i.e. np.exp(-746) == 0.
    ...
    def __init__(self,
        ...
        self._pref_init = 0 # TODO: Separate out
    ...
    cpdef update(self, Agent agent, Action real_action, Observation real_observation,
        ...
        vnode = RootVNodeParticles.from_vnode(VNodeParticles(belief=Particles(copy.deepcopy(particles))),
                                                   agent.history)
    ...
    #         if root[action].num_visits == 0: # TODO: Probably redundant.
    #             val = float('inf')
    #         else: # TODO: Hardcoded constant
    #             val = np.exp(root[action].pref * self._temperature) * 5000 + \
    #                 np.sqrt(np.log(root.num_visits + 1) / root[action].num_visits)
    #             # print(val) # TODO: Remove.
    #         if val > best_value:
    #             best_action = action
    #             best_value = val
    #     return best_action
    ...
    cdef Action _sample_policy(self, State state, VNode root):
        ...
        # Option 3 (UCB)
        # TODO: rename _exploitation_prob to re-search tree?
        # if np.random.random() <= self._exploitation_prob and len(root.children):
        ...
        # Option 4
        # cdef Action action
        # if np.random.random() <= self._exploitation_prob and len(root.children) or len(root.children) >= self._action_branching_factor:
        #     return root.sample_softmax(self._exploitation_temp, "pref")
        # else:
        #     self._global_objective = self._agent._policy_model.maintain_global_objective(state, self._global_objective)
    ...
    # TODO: Old version -- single simulate.
    ...
    #
    #     # Update belief (except at the root node).
    #     if depth >= 1 and root is not None:
    #         root.belief.add(state)
    #     action = self._sample_policy(state, root)
    #
    #     # NB: Resampling is essential when sampling the reference policy as the action can
    #     # betray information about the state particle.
    ...
    #     # root.num_visits += 1
    #     # root[action].num_visits += 1
    #     # root[action].D += (reward * self._reward_scale
    #     #                   + self._discount_factor * self._simulate(next_state,
    #     #                                    history + ((action, observation),),
    #     #                                    root[action][observation],
    #     #                                    root[action],
    #     #                                    observation,
    #     #                                    depth + nsteps) - root[action].D) / root[action].num_visits
    #     # root[action].pref = root[action].pref - root.V + root[action].D
    #
    #     # # Option 3
    #     # root.num_visits += 1
    #     # root[action].num_visits += 1
    ...
    #
    #
    #     root.V = root.max_pref if self._temperature == np.inf else root.mellowmax(self._temperature)
    #
    #     if depth == 0:
    #         print("V |", root.V)
    #     # print("V |", root.V)
    ...
    #
    #     for i in range(self._action_branching_factor):
    #         state = vnode.belief.random()
    #         action = self._agent._policy_model.sample(state, self._agent._policy_model.new_global_objective())
    #         if vnode[action] is None:
    #             vnode[action] = QNode(pref=self._pref_init)
    ...
    cpdef float _simulate(PORPI self,
        ...
        for i in range(self._action_branching_factor):
            state = root.belief.random()
            # objective = self._agent._policy_model.new_global_objective()
            # action = self._agent._policy_model.sample(state, objective)
            self._global_objective = self._agent._policy_model.maintain_global_objective(state, self._global_objective)
            action = self._agent._policy_model.sample(state, self._global_objective)

            if root[action] is None:
                root[action] = QNode(pref=self._pref_init)
                # print(f"New action node added.")
                # print(f"{root.children_data()}")

        action = root.sample_softmax(self._exploitation_temp, "pref") if np.random.random() <= self._exploitation_prob else root.least_visited()
        ...
        #
        #     # sampling importance resampling
        #     for a, o in zip(action.action_sequence, observation.observation_sequence):
        #         resampled_particles = self._sir(self._agent, a, o, particles,
        #                               self._sir_temp, 10)
        #
        # else:
        #     resampled_particles = self._sir(self._agent, action, observation, particles,
        #                           self._sir_temp, 10)
        ...
        # Option 3
        # root[action].num_visits += 1
        # root[action].pref = root[action].pref - root.V + reward \
        #                     + (self.discount_factor ** nsteps) \
        #                     * self._simulate(next_state,
        #                                       history + ((action, observation),),
        #                                       root[action][observation],
        ...
        if depth == 0:
            print(f"Pref: {root[action].pref} | num_visits: {root[action].num_visits}")
        # print(f"Pref: {root[action].pref} | num_visits: {root[action].num_visits} | d: {depth}")
        ...
        # if depth == 0:
        #     print(f"V: {root.V} | num_visits: {root.num_visits} | # children: {len(root.children)}")
        # print(f"V: {root.V} | num_visits: {root.num_visits} | d: {depth} | # children: {len(root.children)}")
