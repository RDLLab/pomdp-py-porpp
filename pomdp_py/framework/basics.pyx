...
cdef class Objective:
    """An Objective is an (ordered) list of states, combined with a dictionary
    of arguments and their values representing the objective's status."""
    ...
    def __str__(self):
        return f"<states: {self.states} | kwargs: {self.kwargs}>"

    def __repr__(self):
        return self.__str__()

cdef class ReferencePolicyModel(PolicyModel):
    """A PolicyModel with the additional capabilities of generating
    new (global) objectives and sampling actions based on the objective.
    By "global" we mean an objective that does not re-seed at every repeated
    call of the reference policy but rather gets updated given an update condition."""

    def new_global_objective(self, state, objective):
        """
        new_global_objective(self, state)
        Returns a new global objective based on the current state (if required).
        """

    def maintain_global_objective(self, state, objective):
        """
        maintain_global_objective(self, state, objective)
        Maintains a global objective based on current state and objective's status.
        """

    def value_heuristic(self, state, next_state, reward, discount):
        raise NotImplementedError

    def sample(self, state, objective=None):
        """
        sample(self, state, objective)
        Returns an action according to the reference policy's specifications and
        a (possibly global) objective.
        """
        raise NotImplementedError
...
cdef class MPPOMDP(POMDP):
    """
    A motion planning POMDP instance.
    """

    def visualize_belief(self, histogram, timeout, life_time, rgb):
        raise NotImplementedError
...
cdef class MacroAction(Action):
    ...
    def __init__(self, action_sequence):
        """
        observation_sequence (list): A list of primitive actions
        """
...
cdef class Agent:
    ...
    def observation_model(self, value):
        self._observation_model = value
    ...
    @reward_model.setter
    def reward_model(self, value):
        self._reward_model = value
    ...
    @blackbox_model.setter
    def blackbox_model(self, value):
        self._blackbox_model = value
    ...
    if isinstance(action, Option):
        ...
        if O is not None:
            for i, a in enumerate(macro_action.action_sequence):
                next_state = T.sample(state, a)
                observation = O.sample(next_state, a)
                observation_sequence.append(observation)
                reward = R.sample(state, a, next_state)
                macro_reward += discount_factor ** i * reward
                state = next_state
                nsteps += 1
            return state, observation_sequence, macro_reward, nsteps
        else:
            for i, a in enumerate(macro_action.action_sequence):
                next_state = T.sample(state, a)
                reward = R.sample(state, a, next_state)
                ...
                state = next_state
                nsteps += 1
            return state, macro_reward, nsteps
