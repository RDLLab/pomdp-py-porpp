...
cdef class PilotState:

    EPSILON = 0.1
    cdef public float _stress
    ...
    def __eq__(self, other):
        if not isinstance(other, PilotState):
            return False
        return np.abs(self._stress - other._stress) < self.EPSILON

    def __str__(self):
        return f"{self.stress}"
...
cdef class PilotObservation:

    EPSILON = 0.5
    cdef public float _obs_char

    def __init__(self, obs_char):
        self._obs_char = obs_char

    def __eq__(self, other):
        if not isinstance(other, PilotObservation):
            return False
        return np.abs(self._obs_char - other._obs_char) < self.EPSILON

    def __str__(self):
        return f"{self.obs_char}"
    ...
    @property
    def obs_char(self):
        return self._obs_char


cdef class Nav3DState(State):

    EPSILON = 1.5 # For histogram.

    cdef public PilotState _pilot_state
    ...
    def __init__(self, pilot_state, position, collision, no_fly_zone, landmark, objectives_reached, goal):
        """
        pilot_state (PilotState): The pilot's true stress level.
    ...
    def __eq__(self, other):
        if not isinstance(other, Nav3DState):
            return False
        return (self._pilot_state == other._pilot_state and
                np.linalg.norm(np.array(self._position) - np.array(other._position), ord=2) < Nav3DState.EPSILON and
    ...
    def __str__(self):
        return (f"<pilot stress: {self._pilot_state} "
...
cdef class Nav3DObservation(Observation):
    ...
    EPSILON=1.0
    cdef public PilotObservation _pilot_observation

    cdef public tuple _pos_reading

    def __init__(self, pilot_observation, pos_reading):
        """
        pilot_observation: The observed pilot characteristics.
        pos_reading: The position reading of the observation.
        """

        self._pilot_observation = pilot_observation
    ...
    def __eq__(self, other):
        if isinstance(other, Nav3DObservation):
            return (np.linalg.norm(self.to_vector - other.to_vector, ord=2) < self.EPSILON and
                    self._pilot_observation == other._pilot_observation)

        return False

    def __str__(self):
        return f"<pilot: {round(self._pilot_observation._obs_char, 2)} | pos: {np.round(self.to_vector, 2)}>"
...
cdef class Nav3DMacroObservation(MacroObservation):
    ...
    def __str__(self):
        return f"<obs_sequence: {[o for o in self.observation_sequence]}>"
...
cdef class Nav3DTransitionModel(TransitionModel):
    ...
    STRESS_LOWER_LIM = 1.
    STRESS_UPPER_LIM = 10.
    STEP_SCALE = 10. # i.e. the rate parameter for the exponential distribution.
    ...
    cpdef sample(self, Nav3DState state, Nav3DAction action):
        ...
        covariance = list(np.array([[1, 0, 0],
                                    [0, 1, 0],
                                    [0, 0, 1]]) * 0.1 * self.STEP_SIZE * state.pilot_state.stress)
        ...
        dv = self.STEP_SIZE * np.array([np.cos(action._latitude) * np.cos(action._longitude),
                                       ...
                                       np.sin(action._latitude)]) + pomdp_py.Gaussian(mean=self.MEAN,
        ...
        pm = (1. if self._pyb_env.dz_checker(state._position + (0, 0, 0)) else -1.) * scipy.stats.expon.rvs(loc=0., scale=self.STEP_SCALE, size=1)[0]
        stress = np.clip(state.pilot_state.stress + self.STRESS_STEP * pm,
                         self.STRESS_LOWER_LIM, self.STRESS_UPPER_LIM)

        return Nav3DState(pilot_state=PilotState(stress=stress),
...
cdef class Nav3DObservationModel(ObservationModel):
    ...
    HELI_OBS_ERROR_DIST = pomdp_py.Gaussian(mean=MEAN, cov=COV)

    PILOT_OBS_MEAN = 0.
    PILOT_OBS_SD = 3.0

    PILOT_OBS_ERROR_DIST = scipy.stats.norm()
    ...
    def probability(self, Nav3DObservation observation, Nav3DState next_state, Nav3DAction action):
        ...
        return (self.HELI_OBS_ERROR_DIST[np.array(observation._pos_reading) - np.array(next_state._position)] *
                self.PILOT_OBS_ERROR_DIST.pdf(observation._pilot_observation.obs_char - next_state.pilot_state.stress - self.PILOT_OBS_MEAN)/self.PILOT_OBS_SD)

    def sample(self, Nav3DState next_state, Nav3DAction action):

        cdef PilotObservation pilot_noisy_reading
        cdef tuple heli_noisy_reading

        pilot_noisy_reading = PilotObservation(next_state.pilot_state.stress + np.random.normal(self.PILOT_OBS_MEAN, self.PILOT_OBS_SD,1)[0])
        heli_noisy_reading = tuple(np.array(next_state._position) + np.array(self.HELI_OBS_ERROR_DIST.random()))

        return Nav3DObservation(pilot_noisy_reading, heli_noisy_reading)
...
cdef class Nav3DRewardModel(RewardModel):
    ...
    MISSION_ACCOMPLISHED_REWARD = 2000  # The reward for accomplishing the mission.
...
class Nav3DMacroReferencePolicyModel(PolicyModel):
    ...
    def __init__(self, pyb_env: PyBulletEnv, max_macro_action_length=1,
        ...
        self._prm = PRM(pyb_env,
                            ...
                            gaussian_noise=10,
                            nodes_to_include=key_positions + gap)
        self._waypoints = self.sample_waypoints()
    ...
    def target_reached(self, Nav3DState state, tuple target):
        return np.linalg.norm(np.array(state._position + (0,0,0)) - np.array(target)) < self.REACHED_DIST
    ...
    def get_new_target(self, Nav3DState state):
        ...
        if len(unvisited_objectives):
            ...
            return state._objectives_reached[-1]
    ...
    def sample_waypoints(self, num_waypoints=0, center=(0,0,0), half_extent=80):
        waypoints = [(30, 60, 60, 0, 0, 0), (30, -60, 60, 0, 0, 0)]
        while len(waypoints) < num_waypoints:
            waypoint = tuple(np.array(center + (0,0,0)) + np.array(self.get_uniform_box_noise(half_extent)))
            nearest_d, nearest_i = self._prm.kd_tree.query(waypoint)
            waypoints.append(tuple(self._prm.kd_tree.data[nearest_i]))
        return waypoints

    # def get_new_waypoint(self, Nav3DState state, tuple target):
    #     return self._waypoints[np.random.randint(len(self._waypoints))]

    # def get_new_waypoint(self, Nav3DState state, tuple target):
    #     """Compute the distance r between the current position and the target and
    #     sample a point some distance k * r from the target.
    ...
    #
    #     target_distance = np.linalg.norm(np.array(state._position+(0,0,0)) - np.array(target))
    #     # TODO: Hardcoded scaling factor.
    #     waypoint = tuple(np.array(target) + np.array(self.get_uniform_ball_noise(target_distance * 1.25)))
    #     nearest_d, nearest_i = self._prm.kd_tree.query(waypoint)
    ...
    def get_new_waypoint(self, Nav3DState state, tuple target):
        ...
        midpoint = tuple(0.5 * (np.array(state._position + (0,0,0)) + np.array(target)))
        midpoint_distance = np.linalg.norm(np.array(midpoint) - np.array(target))
        # TODO: Hardcoded scaling factor.
        # waypoint = tuple(np.array(midpoint) + np.array(self.get_uniform_ball_noise(midpoint_distance * 1.0)))
        waypoint = tuple(np.array(midpoint) + np.array(self.get_uniform_box_noise(midpoint_distance * 1.0)))
        nearest_d, nearest_i = self._prm.kd_tree.query(waypoint)
        return tuple(self._prm.kd_tree.data[nearest_i])
    ...
    #     midpoint_distance = np.linalg.norm(np.array(midpoint) - np.array(target))
    #     # TODO: Hardcoded scaling factor.
    #     waypoint = tuple(np.array(midpoint) + np.array(self.get_uniform_ball_noise(midpoint_distance * 1.2 * 1.0)))
    #     # waypoint = tuple(np.array(midpoint) + np.array(self.get_uniform_box_noise(midpoint_distance * 1.2 * 1.0)))
    #     nearest_d, nearest_i = self._prm.kd_tree.query(waypoint)
    #     return tuple(self._prm.kd_tree.data[nearest_i])
    ...
    #     """
    #     waypoint = tuple(np.random.uniform(low=lower_bounds, high=upper_bounds)) + (0., 0., 0.waypoint)
    #     nearest_d, nearest_i = self._prm.kd_tree.query(waypoint)
    #     return tuple(self._prm.kd_tree.data[nearest_i])
    ...
    def distance_to_goal(self, Nav3DState state):
        ...
        for goal in unvisited_goals:
            ...
            dist_to_goal = np.linalg.norm(np.array(state._position) - np.array(goal[0:3]))
...
class HolonomicRescueStressPOMDP(MPPOMDP):
    ...
    def update_environment(self, env_index=0):
        ...
        if self._pyb_env._id == pyb.GUI:
            ...
            pyb_env_gui = CorsicaDZ(gui=True, debugger=False, dz_level=env_index, track_robot=False)
            self._pyb_env = pyb_env_gui
            self._pyb_env_gui = pyb_env_gui
        else:
            pyb.disconnect(self._pyb_env._id)
            pyb_env = CorsicaDZ(gui=False, debugger=False, dz_level=env_index, track_robot=False)
            pyb.disconnect(self._pyb_env_gui._id)
            pyb_env_gui = CorsicaDZ(gui=False, debugger=False, dz_level=env_index, track_robot=False)
            # pyb_env_gui = CorsicaDZ(gui=True, debugger=False, dz_level=env_index, track_robot=False)
            self._pyb_env = pyb_env
            self._pyb_env_gui = pyb_env_gui
