...
class LogReplayer:
    ...
    STEP_COMPLETE_PATTERN =    "Planner (Time):      "
    ...
    def next_step_data(self, fp: typing.TextIO):
        line = fp.readline()
        end_reached = not line

        if end_reached:
            return None, None, None, None, None, None, end_reached

        bh, sh, a, nsh, o, r = None, None, None, None, None, None
        step_complete = False

        while not all([bh, sh, a, nsh, o, r, step_complete]) and not end_reached:
            line = fp.readline()
        ...
        return bh, sh, a, nsh, o, r, end_reached
    ...
    def replay_log(self, log_file,
        ...
        bel_timeout (float): The timeout before the GUI stops plotting belief particles.
        ...
        with log_file_path.open() as fp:
            while True:
                bh, sh, a, nsh, o, r, end_reached = self.next_step_data(fp)
                ...
                if bespoke_parser is None:
                    ...
                    s = re.findall("dz: [A-Za-z]*", sh)[0].replace("dz: ", "")
                    s_dz = eval(s)
                ...
                # TODO: Add holonomic rescue. elif bespoke_parser == "holonomic_rescue":
                elif bespoke_parser == "drone_capture":
                    from pomdp_py.problems.motion_planning.drone_capture.drone_capture import PPState

                    bel_prey_positions = eval(re.sub("\]", "", re.sub("<prey: \[", "", re.sub(" \| pred: [\[(-.,0-9\]\s]* \| captured: [\[01FalseTrue\]]*>", "", bh))))
                    b = {PPState([(0, 0, 0)]*4, pos, [0]): p for pos, p in bel_prey_positions.items()}
                    s_predators = eval(re.sub(" \| captured: \[[01FalseTrue]*\]>", "", re.sub("<prey: \[[0-9\-,.)(\s]*\] \| pred: ", "", sh)))
                    s_prey = list(eval(re.sub("<prey: ", "", re.sub(" \| pred:.*", "", sh))))
                    s_captured = re.sub(">", "", re.sub(".* \| captured: ", "", sh))

                    ns_predators = eval(re.sub(" \| captured: \[[01FalseTrue]*\]>", "", re.sub("<prey: \[[0-9\-,.)(\s]*\] \| pred: ", "", nsh)))
                ...
                colors = ((1, 1, 1), (0, 1, 1), (0, 1, 0), (0, 0, 1))
                ...
                if belief:
                    ...
                    self.problem.visualize_belief(bel_histogram, step, life_time=bel_viz_life_time, timeout=bel_timeout)

                self.problem.env.apply_transition(ns)

                if trace_trajectory:
                    for prev_xyz, xyz, rgb in zip(s._predator_positions, ns._predator_positions, colors):
                        pyb.addUserDebugLine(lineFromXYZ=prev_xyz, lineToXYZ=xyz,
                                         lineColorRGB=rgb, lineWidth=trajectory_width, lifeTime=0,
    ...
    def replay_logs(self, log_files,
        ...
        start_on_user_prompt (ool): If True, only starts replay after user prompt.
        delay (int): The delay between replay steps.
        step_thru (bool): If True, step through the log file upon user input.
            Otherwise, step through automatically based at the delay speed.
        trace_trajectory (bool): If True, traces the executed trajectory.
