...
class Alps(PyBulletEnv):
    ...
    def load_terrain_collision_objects(self):
        ...
        scale = 500
        meshes = [
            [pathlib.Path(__file__).parent.parent.parent.parent / "data/alps.obj",
    ...
    def load_primitive_objects(self, **kwargs):
        ...
        if texture:
            for obj_id in objects:
                texture_id = pyb.loadTexture(str(texture))
                pyb.changeVisualShape(objectUniqueId=obj_id, linkIndex=-1, textureUniqueId=texture_id)
        else:
            for obj_id in objects:
                pyb.changeVisualShape(objectUniqueId=obj_id, linkIndex=-1, textureUniqueId=-1, rgbaColor=rgba)
...
def main():
    ...
    env_gui = Alps(gui=True, debugger=True, dz_level=2, camera_yaw=-350)
