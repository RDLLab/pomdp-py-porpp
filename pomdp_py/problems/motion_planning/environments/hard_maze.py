...
class HardMaze(PyBulletEnv):
    ...
    def load_terrain_collision_objects(self):
        ...
        heightfields = [
                [pathlib.Path(__file__).parent.parent.parent.parent / "data/maze.png",
                 (1. / 256 * 100., 1. / 256 * 100., 1 / 0.1),
                 (-83600, -380300, 0)]
            ]
        ...
        urdf_files = [
            # [pathlib.Path(__file__).parent.parent.parent.parent / "data/plane.urdf", [0, 0, 0], .01]
        ]
    ...
    def sample_key_configs(self):
        ...
        return tuple(self.key_configs[np.random.randint(len(self.key_configs))])

    def sample_goal_configs(self):
        return tuple(self.goal_configs[np.random.randint(len(self.key_configs))])


def main():

    env_gui = HardMaze(gui=True, debugger=True)
