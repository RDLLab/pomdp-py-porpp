...
class Maze2D(PyBulletEnv):
    ...
    KEY_CONFIGS = LANDMARKS + GOAL_CONFIGS
    ...
    def load_goals(self, visualize=True):
        ...
        return self.load_primitive_objects(spheres=spheres, rgba=[0, 1, 1, 0.9])

    def load_landmarks(self):

        boxes = [
            [[1, 1, 1.5], [22.0, 15.0, 0.0], [0, 0, 0]],
            [[1, 1, 1.5], [0.0, 10.0, 0.0], [0, 0, 0]],
            [[1, 1, 1.5], [19.0, 21.5, 0.0], [0, 0, 0]],
            [[1, 1, 1.5], [-6.0, 14.0, 0.0], [0, 0, 0]],
            [[1, 1, 1.5], [-6.0, 22.0, 0.0], [0, 0, 0]],
            [[1, 1, 1.5], [-7.0, -10.0, 0.0], [0, 0, 0]],
            [[1, 1, 1.5], [-12.0, 2.0, 0.0], [0, 0, 0]],
            [[1.5, 1.5, 1.5], [-16.0, 8.0, 0.0], [0, 0, 0]],
        ]

        return self.load_primitive_objects(boxes=boxes, rgba=[1, 0.1, 1, 0.9])

    def load_danger_zones(self):

        boxes = [
            [[7, 1, 1.5], [6.0, -9.0, 0.], [0, 0, 0]],
            [[7, 1, 1.5], [6.0, -13.0, 0.], [0, 0, 0]],
            [[5, 0.8, 1.5], [6.0, 21.0, 0.], [0, 0, 0]],
            [[0.3, 2.5, 1.5], [-23, 0.5, 0.0], [0, 0, 0]],
        ]

        return self.load_primitive_objects(boxes=boxes, rgba=[1, 1, 0.2, 0.9])

    def load_collision_objects(self):

        heightfields = [
                [pathlib.Path(__file__).parent.parent.parent.parent / "data/gridmap/navigation.png",
                 pathlib.Path(__file__).parent.parent.parent.parent / "data/gridmap/blank.png",
                (0., 0., -3. / 2.),
                (1. / 256 * 50., 1. / 256 * 50., 1. / 3.)]
            ]
    ...
    def load_primitive_objects(self, **kwargs):
        ...
        for file_path, texture_path, center, mesh_scale in heightfields:

            mesh_scale_temp = list(mesh_scale)
            mesh_scale_temp[2] = (1. / mesh_scale_temp[2])
            center_temp = list(center)
            center_temp[2] = center_temp[2] + (mesh_scale_temp[2] / 2)

            shape = pyb.createCollisionShape(
                shapeType=pyb.GEOM_HEIGHTFIELD,
                meshScale=mesh_scale_temp,
                fileName=str(file_path),
                physicsClientId=self._id
            )

            shape_id = pyb.createMultiBody(0, shape, basePosition=center_temp, physicsClientId=self._id)
            if texture_path:
                texture_id = pyb.loadTexture(str(file_path))
                pyb.changeVisualShape(shape_id, -1, textureUniqueId=texture_id)
...
def main():
    ...
    env_gui = Maze2D(gui=True, debugger=True)

    # create user debug parameters
    robot_params_gui = create_robot_params_gui(env_gui._id, start_vals=(21.5, -11.5, 0.0, 0, 0, 0))
