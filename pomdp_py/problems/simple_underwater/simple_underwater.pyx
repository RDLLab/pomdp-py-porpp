...
cdef class Nav3DAction(Action):
    ...
    def __hash__(self):
        return hash(self._dir)

    def __eq__(self, other):
        if isinstance(other, Nav3DAction):
            return self._dir == other._dir

    def __str__(self):
        return f"<dir: {self._dir}>"
...
cdef class Nav3DMacroAction(MacroAction):
    ...
    def __str__(self):
        return f"<action_sequence: {[a._dir for a in self._action_sequence]}>"
...
cdef class Nav3DMacroObservation(MacroObservation):
    ...
    def __init__(self, observation_sequence):
        ...
        self._hash = hash(tuple(observation_sequence))

    def __hash__(self):
        return self._hash

    def __eq__(self, Nav3DMacroObservation other):
        if isinstance(other, Nav3DMacroObservation):
            return self._hash == other._hash
        return False

    def __str__(self):
        return f"<observation_sequence: {[o._pos_reading for o in self._observation_sequence]}>"
...
cdef class Nav3DRewardModel(RewardModel):
    ...
    DZ_REWARD = -1000  # The collision penalty.
    GOAL_REWARD = 5000  # The goal reward.
